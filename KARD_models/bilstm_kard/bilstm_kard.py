...
class BiLSTMClassifier(nn.Module):
    def __init__(self, input_size, hidden_size=32, num_layers=1, num_classes=18, dropout=0.45):
        super().__init__()
...
for epoch in range(20):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        torch.save(model.state_dict(), "kard_bilstm_best.pt")
        counter = 0
    else:
        counter += 1
        if counter >= patience:
...
# =====================================================
model.load_state_dict(torch.load("kard_bilstm_best.pt"))
model.eval()
...
#  Save Outputs
# =====================================================
torch.save(model.state_dict(), "kard_bilstm_final.pt")
with open("kard_bilstm_training_history.json", "w") as f:
    json.dump(history, f)
pickle.dump({"labels": np.unique(y_train)}, open("kard_bilstm_labels.pkl", "wb"))

print("\n Training completed successfully.")
print("Saved:")
print("  • kard_bilstm_best.pt")
print("  • kard_bilstm_final.pt")
print("  • kard_bilstm_training_history.json")
print("  • kard_bilstm_labels.pkl")

# =====================================================
#  Evaluation, Curves, Inference Time, Model Size
# =====================================================
with open("kard_bilstm_training_history.json", "r") as f:
    history = json.load(f)
with open("kard_bilstm_labels.pkl", "rb") as f:
...
# Model size
model_size = os.path.getsize("kard_bilstm_best.pt") / (1024 ** 2)
