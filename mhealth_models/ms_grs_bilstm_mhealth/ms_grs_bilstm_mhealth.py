...
class MultiScaleGatedResidualSkipBiLSTM(nn.Module):
    """
    Multi-Scale Gated Residual Skip BiLSTM (MS-GRS-BiLSTM)
    ------------------------------------------------------
    Captures temporal dependencies at multiple scales using
    three BiLSTM branches:
        1. Normal scale (original temporal resolution)
        2. Downsampled by 2 (medium-term patterns)
        3. Downsampled by 4 (long-term patterns)
    Combines these representations using gated residual fusion.
    """
    def __init__(self, input_size, hidden_size, num_classes, dropout=0.5):
        super().__init__()

        # Three BiLSTM branches for different temporal scales
        self.bilstm_normal = nn.LSTM(
        ...
        # Gating mechanism for multi-scale fusion
    ...
    def forward(self, x):
        ...
        """
        # Scale 1 (normal)
        out_normal, _ = self.bilstm_normal(x)

        # Scale 2 (downsampled by 2)
        x_down2 = x[:, ::2, :]
        out_down2, _ = self.bilstm_down2(x_down2)
        out_down2 = torch.nn.functional.interpolate(
            out_down2.transpose(1, 2), size=out_normal.size(1), mode='linear', align_corners=False
        ).transpose(1, 2)

        # Scale 3 (downsampled by 4)
        x_down4 = x[:, ::4, :]
        out_down4, _ = self.bilstm_down4(x_down4)
        out_down4 = torch.nn.functional.interpolate(
            out_down4.transpose(1, 2), size=out_normal.size(1), mode='linear', align_corners=False
        ).transpose(1, 2)

        # Concatenate multi-scale outputs
        combined = torch.cat((out_normal, out_down2, out_down4), dim=-1)

        # Gated fusion
        gate_values = self.gate(combined)
        fused = gate_values * out_normal + (1 - gate_values) * (
            0.5 * out_down2 + 0.5 * out_down4
        )

        # Many-to-one output (use last timestep)
        last_out = fused[:, -1, :]
...
model = MultiScaleGatedResidualSkipBiLSTM(input_size, hidden_size, num_classes).to(device)
