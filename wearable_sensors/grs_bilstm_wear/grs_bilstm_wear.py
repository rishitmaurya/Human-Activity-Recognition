...
output_dir = r'wearable_sensors/grs_bilstm_wear/testing'
...
def create_windows(X, y, window_size=128, step_size=32):
    ...
    for i in range(0, len(X) - window_size, step_size):
        ...
        label = np.argmax(np.bincount(y[i:i+window_size]))
        X_win.append(win)
        y_win.append(label)
...
# =========================================================
# 5. MODEL ARCHITECTURE (GRS-BiLSTM)
# =========================================================
class GRSBiLSTM(nn.Module):
    def __init__(self, num_classes):
        super().__init__()

        self.conv = nn.Conv1d(8, 192, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        base = torch.relu(self.bn(self.conv(x)))
        gate = torch.sigmoid(self.gate(base))
        x = torch.relu(base * gate + self.shortcut(x))
...
model = GRSBiLSTM(num_classes).to(device)
...
for epoch in range(50):
    ...
    print(f"Epoch {epoch+1}/50 | Acc={history['accuracy'][-1]:.4f} | Val={history['val_accuracy'][-1]:.4f}")
