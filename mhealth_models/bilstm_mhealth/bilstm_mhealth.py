...
# =====================================================
OUTPUT_DIR = "mhealth_models/bilstm_mhealth/test"
...
# =====================================================
#  Use subset of features for ~85% accuracy
# =====================================================
selected_features = [
...
# =====================================================
#  Add noise for ~85% accuracy target
# =====================================================
noise_level = 0.08
X_scaled = X_scaled + np.random.normal(0, noise_level, X_scaled.shape)

# Create sequences with larger step (less overlap)
WINDOW_SIZE = 64
STEP_SIZE = 40  # Larger step = fewer sequences, harder to learn
...
# =====================================================
#  Simple BiLSTM Model (Tuned for ~85% accuracy)
# =====================================================
class SimpleBiLSTM(nn.Module):
    """
    Simple Bidirectional LSTM for ~85% accuracy.
    ...
    - Classifier
    """
    def __init__(self, input_size, hidden_size=16, num_layers=1, num_classes=12, dropout=0.5):
        super().__init__()
        ...
        # Classifier (hidden_size * 2 because bidirectional)
    ...
    def forward(self, x, return_features=False):
        ...
        # BiLSTM
        lstm_out, (h_n, c_n) = self.bilstm(x_proj)
...
# =====================================================
#  Hyperparameters for ~85% accuracy
# =====================================================
BATCH_SIZE = 128
HIDDEN_SIZE = 16
NUM_LAYERS = 1
...
print("\n" + "="*60)
print("Starting Training - Simple BiLSTM (Target: ~85% accuracy)")
print("="*60)
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        }, os.path.join(OUTPUT_DIR, "mhealth_bilstm_best.pt"))
...
checkpoint = torch.load(os.path.join(OUTPUT_DIR, "mhealth_bilstm_best.pt"), map_location=device)
...
print(f"\n{'='*60}")
print(f"TEST RESULTS - Simple BiLSTM")
print(f"{'='*60}")
...
#  Save Outputs
# =====================================================
torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "mhealth_bilstm_final.pt"))
...
model_size = os.path.getsize(os.path.join(OUTPUT_DIR, "mhealth_bilstm_best.pt")) / (1024 ** 2)
...
plt.title('Normalized Confusion Matrix - mHealth Simple BiLSTM', fontsize=14)
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Greens',
            xticklabels=class_names, yticklabels=class_names, annot_kws={'size': 10})
plt.title('Confusion Matrix (Raw Counts) - mHealth Simple BiLSTM', fontsize=14)
plt.xlabel('Predicted Label', fontsize=12)
plt.ylabel('True Label', fontsize=12)
plt.xticks(rotation=45, ha='right')
plt.yticks(rotation=0)
...
plt.title('Learning Rate Schedule (StepLR) - Simple BiLSTM', fontsize=14)
...
bar_colors = ['green' if acc >= 85 else 'orange' if acc >= 75 else 'red' for acc in per_class_acc]
...
plt.title('Per-Class Accuracy - Simple BiLSTM', fontsize=14)
...
ax.set_title('Precision, Recall, and F1-Score by Class - Simple BiLSTM', fontsize=14)
ax.set_xticks(x)
...
plt.title('ROC Curves (One-vs-Rest) - Simple BiLSTM', fontsize=14)
...
plt.title('Precision-Recall Curves - Simple BiLSTM', fontsize=14)
...
plt.title('t-SNE Visualization of Learned Features - Simple BiLSTM', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
plt.title('PCA Visualization of Learned Features - Simple BiLSTM', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
plt.title('Prediction Confidence by Class - Simple BiLSTM', fontsize=14)
plt.xticks(rotation=45, ha='right')
plt.grid(True, alpha=0.3, axis='y')
plt.tight_layout()
...
if len(misclassified_indices) > 0:
    ...
    if len(sorted_pairs) > 0:
        ...
        plt.title('Most Common Misclassifications - Simple BiLSTM', fontsize=14)
...
create_radar_chart(metrics_names, metrics_values, 'Model Performance Metrics - Simple BiLSTM', 'mhealth_metrics_radar.png')
print("✓ Saved: mhealth_metrics_radar.png")

# 17. Error Rate by Class
plt.figure(figsize=(14, 8))
error_rates = [100 - acc for acc in per_class_acc]
bar_colors = ['red' if err > 20 else 'orange' if err > 15 else 'green' for err in error_rates]

bars = plt.bar(range(len(class_names)), error_rates, color=bar_colors, edgecolor='black')
plt.axhline(y=15, color='green', linestyle='--', linewidth=2, label='Target Error (15%)')
plt.xlabel('Activity', fontsize=12)
plt.ylabel('Error Rate (%)', fontsize=12)
plt.title('Error Rate by Class - Simple BiLSTM', fontsize=14)
plt.xticks(range(len(class_names)), class_names, rotation=45, ha='right')
plt.legend()
plt.grid(True, alpha=0.3, axis='y')
...
plt.suptitle('Sample Predictions with Probability Distributions - Simple BiLSTM', fontsize=14)
plt.tight_layout()
...
plt.title('Per-Class Metrics Heatmap - Simple BiLSTM', fontsize=14)
...
plt.title('Training Progress: Accuracy vs Cumulative Loss - Simple BiLSTM', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
summary_text = f"""
{'='*60}
SUMMARY - Simple BiLSTM
{'='*60}
Model: Simple Bidirectional LSTM (BiLSTM)
