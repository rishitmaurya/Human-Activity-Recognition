# --- train_uci_lstm.py ---
# LSTM model (simpler than BiLSTM) targeting test accuracy, 50 epochs
...
# =====================================================
OUTPUT_DIR = "uci_models/lstm_uci/test"
...
# Select 100 features randomly - same as before for fair comparison
np.random.seed(123)
NUM_FEATURES_TO_USE = 100
...
# =====================================================
#  MODEL: LSTM (Plain Unidirectional LSTM)
#  No Gated Residual Unit - simplest recurrent baseline
# =====================================================

class LSTM(nn.Module):
    """
    Plain LSTM: Unidirectional LSTM
    
    Simplified architecture compared to BiLSTM:
    - Unidirectional instead of Bidirectional (key difference)
    - No Gated Residual Unit
    ...
    def __init__(self, input_size, hidden_size=64, num_classes=6, dropout=0.55):
        ...
        self.lstm = nn.LSTM(
            hidden_size,
            hidden_size,  # Full hidden size (not divided by 2)
            num_layers=1,
            batch_first=True,
        ...
        # Simple attention (no GRU before it)
    ...
    def forward(self, x, return_features=False):
        ...
        # Simple attention pooling directly on LSTM output
        # (No Gated Residual Unit - key difference from GRS-BiLSTM)
        attn_weights = torch.softmax(self.attention(lstm_out), dim=1)
        features = torch.sum(lstm_out * attn_weights, dim=1)
...
# =====================================================
#  Hyperparameters - same as BiLSTM for fair comparison
# =====================================================
BATCH_SIZE = 64
HIDDEN_SIZE = 32
NUM_LAYERS = 1
...
for epoch in range(NUM_EPOCHS):
    ...
    if avg_val_loss < best_val_loss:
        ...
        }, os.path.join(OUTPUT_DIR, "uci_lstm_best.pt"))
...
checkpoint = torch.load(os.path.join(OUTPUT_DIR, "uci_lstm_best.pt"), map_location=device)
...
#  Save all artifacts
# =====================================================
torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "uci_lstm_final.pt"))
...
model_path = os.path.join(OUTPUT_DIR, "uci_lstm_best.pt")
...
ax.set_title('Training Progress - LSTM', fontsize=16)
...
plt.text(0.5, 0.95, 'LSTM Architecture', fontsize=16, fontweight='bold', 
...
- Fewer parameters than BiLSTM
- Simpler temporal dependencies
...
- 1-layer LSTM (unidirectional, hidden={HIDDEN_SIZE})
