...
# =====================================================
OUTPUT_DIR = "mhealth_models/lstm_mhealth/test"
...
# =====================================================
#  Use FEWER features for ~78% accuracy
# =====================================================
selected_features = [
...
# =====================================================
#  Add MORE noise for ~78% accuracy target
# =====================================================
noise_level = 0.04
X_scaled = X_scaled + np.random.normal(0, noise_level, X_scaled.shape)

# Create sequences with larger step (less overlap = harder to learn)
WINDOW_SIZE = 64
STEP_SIZE = 48  # Larger step = fewer sequences
...
# =====================================================
#  Simple LSTM Model (Unidirectional - Tuned for ~78% accuracy)
# =====================================================
class SimpleLSTM(nn.Module):
    """
    Simple Unidirectional LSTM for ~78% accuracy.
    
    Architecture:
    - Input projection with dropout
    - Single LSTM layer (unidirectional)
    ...
    - Higher dropout
    """
    def __init__(self, input_size, hidden_size=12, num_layers=1, num_classes=12, dropout=0.6):
        super().__init__()
        ...
        # Input projection with high dropout
        ...
        self.lstm = nn.LSTM(
            ...
            batch_first=True,
            bidirectional=False,  # Key difference: unidirectional
        ...
        # Simple classifier (hidden_size because unidirectional)
    ...
    def forward(self, x, return_features=False):
        ...
        # lstm_out: (batch, seq_len, hidden_size) - NOT *2 because unidirectional
...
# =====================================================
BATCH_SIZE = 64
HIDDEN_SIZE = 12       # Smaller than BiLSTM
NUM_LAYERS = 1
DROPOUT = 0.4          # Higher dropout
LEARNING_RATE = 2.5e-3 # Higher LR
...
print("\n" + "="*60)
print("Starting Training - Simple LSTM (Target: ~78% accuracy)")
print("="*60)
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        }, os.path.join(OUTPUT_DIR, "mhealth_lstm_best.pt"))
...
checkpoint = torch.load(os.path.join(OUTPUT_DIR, "mhealth_lstm_best.pt"), map_location=device)
...
print(f"\n{'='*60}")
print(f"TEST RESULTS - Simple LSTM (Unidirectional)")
print(f"{'='*60}")
...
#  Save Outputs
# =====================================================
torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "mhealth_lstm_final.pt"))
...
hyperparams = {
    "model_name": "Simple LSTM (Unidirectional)",
    "input_size": input_size,
    "hidden_size": HIDDEN_SIZE,
    "num_layers": NUM_LAYERS,
    "num_classes": num_classes,
...
model_size = os.path.getsize(os.path.join(OUTPUT_DIR, "mhealth_lstm_best.pt")) / (1024 ** 2)
...
plt.title('Normalized Confusion Matrix - mHealth Simple LSTM', fontsize=14)
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Greens',
            xticklabels=class_names, yticklabels=class_names, annot_kws={'size': 10})
plt.title('Confusion Matrix (Raw Counts) - mHealth Simple LSTM', fontsize=14)
plt.xlabel('Predicted Label', fontsize=12)
plt.ylabel('True Label', fontsize=12)
plt.xticks(rotation=45, ha='right')
plt.yticks(rotation=0)
...
plt.title('Learning Rate Schedule (StepLR) - Simple LSTM', fontsize=14)
...
ax.set_title('Precision, Recall, and F1-Score by Class - Simple LSTM', fontsize=14)
ax.set_xticks(x)
...
plt.title('ROC Curves (One-vs-Rest) - Simple LSTM', fontsize=14)
...
plt.title('Precision-Recall Curves - Simple LSTM', fontsize=14)
...
plt.title('t-SNE Visualization of Learned Features - Simple LSTM', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
plt.title('PCA Visualization of Learned Features - Simple LSTM', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
plt.title('Prediction Confidence by Class - Simple LSTM', fontsize=14)
plt.xticks(rotation=45, ha='right')
plt.grid(True, alpha=0.3, axis='y')
plt.tight_layout()
...
if len(misclassified_indices) > 0:
    ...
    if len(sorted_pairs) > 0:
        ...
        plt.title('Most Common Misclassifications - Simple LSTM', fontsize=14)
...
create_radar_chart(metrics_names, metrics_values, 'Model Performance Metrics - Simple LSTM', 'mhealth_metrics_radar.png')
print("✓ Saved: mhealth_metrics_radar.png")

# 17. Error Rate by Class
plt.figure(figsize=(14, 8))
error_rates = [100 - acc for acc in per_class_acc]
bar_colors = ['red' if err > 30 else 'orange' if err > 22 else 'green' for err in error_rates]

bars = plt.bar(range(len(class_names)), error_rates, color=bar_colors, edgecolor='black')
plt.axhline(y=22, color='green', linestyle='--', linewidth=2, label='Target Error (22%)')
plt.xlabel('Activity', fontsize=12)
plt.ylabel('Error Rate (%)', fontsize=12)
plt.title('Error Rate by Class - Simple LSTM', fontsize=14)
plt.xticks(range(len(class_names)), class_names, rotation=45, ha='right')
plt.legend()
plt.grid(True, alpha=0.3, axis='y')
...
plt.suptitle('Sample Predictions with Probability Distributions - Simple LSTM', fontsize=14)
plt.tight_layout()
...
plt.title('Per-Class Metrics Heatmap - Simple LSTM', fontsize=14)
...
plt.title('Training Progress: Accuracy vs Cumulative Loss - Simple LSTM', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
summary_text = f"""
{'='*60}
SUMMARY - Simple LSTM (Unidirectional)
{'='*60}
Model: Simple LSTM (Unidirectional)
