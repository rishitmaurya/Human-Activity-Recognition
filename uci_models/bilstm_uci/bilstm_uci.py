# --- train_uci_bilstm.py ---
# BiLSTM model (simpler than GRS-BiLSTM) targeting  test accuracy, 50 epochs
...
# =====================================================
OUTPUT_DIR = "uci_models/bilstm_uci/test"
...
# Select 200 features randomly - same as GRS-BiLSTM for fair comparison
np.random.seed(123)
...
# =====================================================
#  MODEL: BiLSTM (Plain Bidirectional LSTM)
#  No Gated Residual Unit - simplest recurrent baseline
# =====================================================

class BiLSTM(nn.Module):
    """
    Plain BiLSTM: Bidirectional LSTM
    
    Simplified architecture compared to GRS-BiLSTM:
    - No Gated Residual Unit (key difference)
    ...
    def forward(self, x, return_features=False):
        ...
        # Simple attention pooling directly on BiLSTM output
...
# =====================================================
#  Hyperparameters - same as GRS-BiLSTM for fair comparison
# =====================================================
BATCH_SIZE = 64
HIDDEN_SIZE = 32
NUM_LAYERS = 1
...
print("\n" + "="*70)
print("Starting Training - BiLSTM")
print("="*70)
...
for epoch in range(NUM_EPOCHS):
    ...
    if avg_val_loss < best_val_loss:
        ...
        }, os.path.join(OUTPUT_DIR, "uci_bilstm_best.pt"))
...
checkpoint = torch.load(os.path.join(OUTPUT_DIR, "uci_bilstm_best.pt"), map_location=device)
...
#  Save all artifacts
# =====================================================
torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "uci_bilstm_final.pt"))
...
model_path = os.path.join(OUTPUT_DIR, "uci_bilstm_best.pt")
...
ax.set_title('Training Progress - BiLSTM', fontsize=16)
...
plt.text(0.5, 0.95, 'BiLSTM Architecture', fontsize=16, fontweight='bold', 
...
Key differences from GRS-BiLSTM:
- NO Gated Residual Unit
- Direct BiLSTM → Attention → Classifier
- Fewer parameters (no gate/residual layers)
